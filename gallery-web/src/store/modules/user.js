...
const SET_LIKE_POST = 'user/SET_LIKE_POST';
const SET_UNLIKE_POST = 'user/SET_UNLIKE_POST';
const CHANGE_USER_PROFILE = 'user/CHANGE_USER_PROFILE';
const SET_ERROR = 'user/SET_ERROR';
const SET_DISABLED = 'user/SET_DISABLED';
const UNSET_DISABLED = 'user/UNSET_DISABLED';
const CHANGE_USERPASSWORD = 'user/CHANGE_USERPASSWORD';
...
export const changeUserProfileImage = createAction(
  ...
  UserAPI.changeUserProfileImage
);
...
export const changeUserProfile = createAction(CHANGE_USER_PROFILE, UserAPI.changeUserProfile);
export const setError = createAction(SET_ERROR);
export const setDisabled = createAction(SET_DISABLED);
export const unsetDisabled = createAction(UNSET_DISABLED);
export const changeUserPassword = createAction(CHANGE_USERPASSWORD, UserAPI.changeUserPassword);
...
export default handleActions(
  {
    ...
    ...pender({
      ...
      onSuccess: (state, action) => {
        ...
        // console.log(isLast);
        return state
          .set('posts', state.get('posts').concat(posts))
    ...
    ...pender({
      ...
      onSuccess: (state, action) => {
        ...
        const isLast = action.payload.headers['is_last'];
        return state
          .set('followingPosts', state.get('followingPosts').concat(followingPosts))
          .set('isLast', isLast === 'True');
      },
    }),
    [SET_FOLLOW]: (state, action) => {
      const { id } = action.payload;
      const updatedFollowing = state
        .get('following')
        .toJS()
        .map((following, index) => {
          if (following.id === id) {
    ...
    [SET_ERROR]: (state, action) => {
      ...
      return state.set('error', true).set('errorMessage', reason);
    },
    [SET_DISABLED]: (state, action) => {
      return state.set('disabled', true);
    },
    [UNSET_DISABLED]: (state, action) => {
